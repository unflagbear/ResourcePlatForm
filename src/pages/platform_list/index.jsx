            ...
            <Card.Meta
              ...
              description={
                ...
                >
                  {item.company}
                  <br />
                  {item.professionDomain}
                  <span>注册日期：{moment(item.registrationTime).format('y-M-d')}</span>
                </Paragraph>
              }
            />
            <div className={styles.cardItemContent}>
              <Tag color="blue">{item.url}</Tag>
              <Tag color="green">地点：{item.registrationAddress}</Tag>
            </div>
          </Card>
        </List.Item>
      )}
          ...
          <StandardFormRow title="其它选项" grid last>
            <Row gutter={16}>
              <Col lg={8} md={10} sm={10} xs={24}>
                <FormItem {...formItemLayout} label="地点" name="place">
                  <Select
                    placeholder="不限"
                    style={{
                      maxWidth: 200,
...
export default connect(({ listAndplatform, loading }) => ({
  listAndplatform,
  loading: loading.models.listAndplatform,
}))(PlatformList);
