...
class Feature6 extends React.PureComponent {
  ...
  getChildrenToRender = (dataSource,numData) => {
    ...
    const childrenToRender = childWrapper.map((item, ii) => {
      ...
      const childrenItem = children.map(($item, i) => {
        const {name, number, children: child, ...childProps } = $item;
        const numberChild = numData?numData[i]:0
  ...
  render() {
    const { dataSource, isMobile,numData, ...props } = this.props;
    return (
      <div {...props} {...dataSource.wrapper}>
        <div>
          <OverPack {...dataSource.OverPack}>
            {this.getChildrenToRender(dataSource,numData)}
          </OverPack>
        </div>
      </div>
    );
